...
ax.barh(y, [1]*len(y), color=OK['blue'], edgecolor='black', lw=.5, height=.60,
        label='coeficiente base $(3,\\mathbf{60})$', zorder=3)
ax.barh(y, [-1]*len(y), color=OK['orange'], edgecolor='black', lw=.5, height=.60,
        label='completacion de 45 multipletes (lo cancela)', zorder=3, alpha=.96)
...
ax.set_xticklabels(['completacion', r'suma $=\mathbf{0}$', 'base'], fontsize=10)
ax.invert_yaxis()
ax.set_title(r'Los ocho coeficientes de anomalia se cancelan (verde $=0$)', fontsize=12)
ax.legend(loc='lower center', bbox_to_anchor=(0.5, -0.20), ncol=2, frameon=True, fontsize=9)
for s in ('top', 'right', 'left'): ax.spines[s].set_visible(False)
ax.tick_params(axis='y', length=0)
fig.tight_layout(); fig.savefig(OUT/'fig_anomaly_cancel_es.pdf')
fig.savefig(OUT/'fig_anomaly_cancel_es.png', dpi=400); plt.close(fig)
...
ax.text(66, -12, 'completacion\n45 multipletes', color=OK['green'], fontsize=9.5,
        rotation=14, ha='center', va='bottom', fontweight='bold')
pts = [((126, -36), 'base $(3,\\mathbf{60})$', OK['blue'], 'FALLA tadpole'),
       ((126, -20), '27 multipletes\n(solo 8 anomalias)', OK['red'], 'FALLA tadpole'),
       ((0, 0),   'objetivo conjunto', OK['green'], 'PASA')]
...
ax.set_xlabel(r'$\Theta_{01}$  (tadpole en $f=0,1$)')
ax.set_ylabel(r'$\Theta_{23}$  (tadpole en $f=2,3$)')
ax.set_title('El tadpole localizado puede cancelarse conjuntamente', fontsize=11.5)
for s in ('top', 'right'): ax.spines[s].set_visible(False)
fig.tight_layout(); fig.savefig(OUT/'fig_tadpole_journey_es.pdf')
fig.savefig(OUT/'fig_tadpole_journey_es.png', dpi=400); plt.close(fig)
...
cname = {'1':r'singlete de color $\mathbf{1}$','3':r'triplete $\mathbf{3}$','3bar':r'antitriplete $\bar{\mathbf{3}}$'}
...
ax.set_xlabel('multiplicidad en la completacion')
ax.set_xlim(0, 9)
...
ax.set_title(r'La completacion de 45 multipletes por rep de color  (total $=45$)', fontsize=11.5)
for s in ('top', 'right'): ax.spines[s].set_visible(False)
ax.grid(axis='x', alpha=.2)
fig.tight_layout(); fig.savefig(OUT/'fig_spectrum45_es.pdf')
fig.savefig(OUT/'fig_spectrum45_es.png', dpi=400); plt.close(fig)
